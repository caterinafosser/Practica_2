...
if (is.na(dia)==TRUE|(dia<1)|(dia>31)){  #chequeo q este entre los valores correctos y que sea un numero
...
while (j<=length(dias_agosto)){
  ...
  if (is.na(t_dia)==TRUE){  #warning si no se ingreso un numero
...
if (is.na(t_media)==TRUE){ #warning si hay algun valor invalido
